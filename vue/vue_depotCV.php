...
<body>
    ...
    <script>
        let cvPath = "<?= $pathCVStudent ?>";
        const preview = document.getElementById('pdfPreview');
        let reader = new FileReader();
        if(cvPath.startsWith("/storage/")){
            if(cvPath.endsWith(".pdf")) preview.innerHTML = `<embed src="${cvPath}" type="application/pdf" width="100%" height="600px" />`;
            else if(cvPath.endsWith(".png") || cvPath.endsWith(".jpeg")) preview.innerHTML = `<img src="${cvPath}" alt="Aperçu de l\'image" style="max-width: 100%; max-height: 600px;" />`;
        }
